...
module.exports = {
  ...
  getFavorite: async (req, res) => {
    try {
      ...
      const recipes = await Favorite.find({ user: req.user.id }).populate('recipe').populate({
        path: 'recipe',
      ...
      console.log(recipes)
      //Sending post data from mongodb and user data to ejs template
      res.render("favorite.ejs", { recipes: recipes, user: req.user });
    } catch (err) {
      console.log(err);
    }
  },
  getFeed: async (req, res) => {
    ...
    try {
      ...
      //console.log(req.user) to see everything
      //Grabbing just the posts of the logged-in user
      const recipes = await Recipe.find().sort({ createdAt: "desc" }).lean().populate('user');
      //Sending post data from mongodb and user data to ejs template
